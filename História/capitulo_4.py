...
def iniciar_combate(jogador, nome_inimigo, hp_inimigo, min_dano, max_dano):
    ...
    while hp_inimigo > 0 and jogador["vitalidade"] > 0:
        ...
        if acao == "1":
            if jogador.get("espada_quebrada", False):
                slow_print("> Sua espada está QUEBRADA! Você bate com o cabo, causando quase nada de dano!")
                ...
                slow_print(f"> A Kagekiri (Sol Negro) incinera a guarda do inimigo! Causou {dano_jogador} de dano bruto.")
                hp_inimigo -= dano_jogador
                ...
                slow_print(f"> A Mizukiri (Lua Prateada) brilha intensamente! A essência do inimigo cura suas feridas (+{cura_lunar} HP).")
                
            else:
                bonus_arma = jogador.get("bonus_dano_arma", 0)
                dano_jogador = random.randint(3, 10) + (jogador.get("kenjutsu", 10) // 3) + bonus_arma
                slow_print(f"> Você desferiu um golpe preciso! Causou {dano_jogador} de dano ao inimigo.")
                hp_inimigo -= dano_jogador
            ...
            if rolar_teste("destreza", jogador.get("destreza", 10), 20):
                slow_print("> Você mergulha na água turva e escapa com vida!")
                return "fuga"
            else:
                slow_print("> Você escorrega na lama e o inimigo bloqueia sua fuga!")
        else:
            slow_print("> Você hesita!")
        ...
        if hp_inimigo > 0:
            ...
            if total_ataque >= defesa_jogador:
                ...
                slow_print(f"> 💥 O golpe rasga sua carne! Você perde {dano_sofrido} de HP.")
                jogador["vitalidade"] -= dano_sofrido
            else:
                if jogador.get("duas_espadas_longas", False):
                    slow_print(f"> ⚔️ Você cruza o Sol e a Lua e defende o impacto esmagador sem dar um passo para trás!")
                else:
                    slow_print(f"> 💨 Você rola pela lama e esquiva do golpe por um triz!")
...
def cena_caminhos_mizu(jogador):
    print("\n" + "="*60)
    slow_print("          OS ARROZAIS TURVOS E O PESO DO FRACASSO")
    print("="*60 + "\n")
    ...
    if escolha_rota == "1":
        ...
        if escolha_dragao == "1":
            if rolar_teste("conhecimento", jogador["conhecimento"], 18):
                ...
                slow_print("FALHA! Sua pergunta é confusa. As cabeças riem: 'Patético!' Elas disparam um jato de água fervente!")
        ...
        if escolha_vila == "1":
            if rolar_teste("destreza", jogador["destreza"], 20):
                ...
                slow_print("FALHA! Um dos sapos nota sua sombra na água e atira um arpão rústico em seu ombro!")
            ...
            if rolar_teste("conhecimento", jogador["conhecimento"], 19):
                ...
                slow_print("FALHA! A palha está molhada demais. O fogo apaga. Você atrai a atenção de três bandidos armados!")
...
def cena_forja_sagrada(jogador):
    ...
    print("\n" + "="*60)
    slow_print("          A CAVERNA DE CRISTAL E AS LÂMINAS GÊMEAS")
    print("="*60 + "\n")
    ...
    slow_print("Você deposita os três estilhaços da Kagekiri na bigorna.")
...
def cena_boss_aquatico(jogador):
    ...
    print("\n" + "="*60)
    slow_print("          A INVASÃO DO SENHOR DAS ÁGUAS TURVAS")
    print("="*60 + "\n")
...
def jogar(jogador):
    print("\n" + "=" * 75)
    slow_print("                   CAPÍTULO 4: AS ÁGUAS DO LAMENTO", 0.05)
    print("=" * 75)
